...
def closesoft():
    ...
    try:
        ...
        wps = win32.gencache.EnsureDispatch('wps.application')
    else:
        wps = win32.gencache.EnsureDispatch('word.application')
...
class App(customtkinter.CTk):
    def __init__(self):
        ...
        self.appearance_mode_optionemenu = customtkinter.CTkOptionMenu(self.sidebar_frame, font=customtkinter.CTkFont(family='Times New Roman', size=15, weight="bold"),
        ...
        self.sign_Combine = customtkinter.CTkButton(master=self.sign_frame, command=lambda: threading.Thread(
                                 target=self.Word_PDF
                             ).start(), font=customtkinter.CTkFont(size=20, weight="bold"), text='运行',
                                                    fg_color="transparent", border_width=2, text_color=("gray10", "#DCE4EE"))
        ...
        self.textbox.insert("0.0", "使用手册：\n\n一、 合并Excel表\n  1. 点击”选择路径“按钮，选择Excel文件所在目录\n  2. 选择表头所在行(数字)\n  3. 选择序号所在列(数字)\n  4. 选择是否统一字体\n  5. 点击运行\n\n"
                                   "二、生成签字表\n  1. 点击”选择文件“按钮，选择示例的签字表文件\n  2. 输入签字表中检测的标题\n  3. 选择项目名称所在列(字母)\n  4. 点击运行")
    ...
    def Word_PDF(self):
        ...
        # 另存为PDF
        doc.SaveAs(os.path.join(result_dir, 'Sign.pdf'), FileFormat=win32.constants.wdFormatPDF)
